...
export function setupScheduler(sessions: Session[]): void {
  for (const session of sessions) {
    ...
    cron.schedule(cronExpr, () => {
      console.log(`[Scheduler] Reset triggered for session: ${session.label} ${session.time}`);
      store.reset();
    }, {
      timezone: 'Europe/Berlin',
    });

    console.log(`[Scheduler] Scheduled reset: ${session.label} at ${session.resetTime} (cron: ${cronExpr})`);
  }
}

// Helper: returns the next upcoming training date/time from sessions
export function getNextTraining(sessions: Session[]): { label: string; date: Date; displayStr: string } {
  const now = new Date();
  ...
  for (const session of sessions) {
    ...
    for (let offset = 0; offset <= 7; offset++) {
      ...
      if (dayOfWeek === session.day && candidate > now) {
        const displayStr = candidate.toLocaleDateString('de-DE', {
          weekday: 'long',
          day: '2-digit',
          month: '2-digit',
          year: 'numeric',
